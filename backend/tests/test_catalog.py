...
def setup_module(module):
    ...
    with Session(engine) as session:
        ...
        for record in (monstera, adansonii, pot, paused, sold_out, hidden_item):
            session.add(record)
        session.commit()
        for record in (monstera, adansonii, paused, sold_out, hidden_item):
...
def test_catalog_facets():
    ...
    assert len(facets["viveros"]) == 1
    # Monstera x2, pot, sold-out plant — the paused one is excluded.
    assert facets["viveros"][0]["item_count"] == 4
    assert data["total"] == 4


def test_catalog_detail_returns_related_same_genus():
    response = client.get(f"/api/catalog/{monstera_id}")  # noqa: F821 - set in setup_module
    assert response.status_code == 200
    data = response.json()
